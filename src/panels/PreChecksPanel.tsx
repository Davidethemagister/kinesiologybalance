...
export function PreChecksPanel() {
  ...
  function handleResult(id: string, result: 'strong' | 'weak') {
    dispatch({ type: 'SET_PRECHECK_RESULT', id, result })
    if (result === 'weak') {
  ...
  function answerEmotionAttached(id: string, attached: boolean) {
    dispatch({ type: 'SET_PRECHECK_EMOTION_ATTACHED', id, attached })
  ...
  function handleEmotionConfirm(entry: EmotionEntry) {
    if (emotionChartForId) {
      dispatch({ type: 'SET_PRECHECK_EMOTION_ENTRY', id: emotionChartForId, entry })
    }
  ...
  function submitCustom() {
    ...
    if (!name) return
    dispatch({ type: 'ADD_CUSTOM_PRECHECK', name })
    setCustomName('')
        ...
        {state.preChecks.map((check) => (
          <div key={check.id} className="bg-white rounded-2xl border border-slate-200 p-4">
            ...
            {expandedNotes.has(check.id) && (
              <textarea
                value={check.notes}
                onChange={(e) => dispatch({ type: 'SET_PRECHECK_NOTES', id: check.id, notes: e.target.value })}
                placeholder="Notes..."
                rows={2}
